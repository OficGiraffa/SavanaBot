...
module.exports.run = async (client, message, args) =>{
     ...
     if(!message.member.hasPermission("MANAGE_ROLES")) return message.reply("Me parece que você não tem permissão :thinking:");
    ...
    if(args[0] === "del") {
    if(rolemute === null) return message.reply("não há nenhum cargo definido nesse servidor pra eu tirar :thinking:")
    let muterole = db.delete(`muterole_${message.guild.id}`) 
    message.reply("cargo removido com sucesso!")
    }
    if(args[0] === "list") {
        if(rolemute === null) return message.reply("não há um cargo definido nesse servidor!")
    ...
    message.channel.send(`OK,apartir de agora "${role}" será o cargo usado para mutar membros deste servidor 👍`)

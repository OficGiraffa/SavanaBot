...
module.exports.run = async (client, message, args, prefix) => {
  ...
  message.channel.send(
    "ATENÇÃO JOGADORES! Vocês devem votar nos jogadores que colocarei aqui usando :white_check_mark:! Tentem achar o único impostor!(Vocês tem 10s)"
  );
  ...
  players.forEach(player => {
    let msg = message.channel.send(`${player}`).then(msg => {
      ...
      reactionCol.on("collect", (reaction, user) => {
        if (!user === user.bot) {
          message.channel.send(
            `<@${user.id}> votou em ${reaction.message.mentions.users.first()}`
          );
        }
      });
    ...
    voted_colls = voted_colls.sort((a, b) => {
      ...
      if (a.first().count === b.first().count) {
        return message.channel.send(
          "Empate! Ninguém foi ejetado! O impostor acabou de matar todo mundo por não tomarem uma decisão! Perderam!"
        );
        empate = true;
      }
      return 0;
        ...
        let msg_embed = new Discord.MessageEmbed()
          ...
          .setDescription(
            ". 　　　。　　　　•　 　ﾟ　　。 　　. \n" +
              ...
              ".　　 。　　　　　 ඞ 。 . 　　 • 　　　　•\n" +
              `　ﾟ　　 <@${
                voted_colls[0].first().message.mentions.users.first().id
              }> não era um impostor　 。　. \n` +
              ` '　　　 Vocês perderam!     　 　　。\n` +
              "　ﾟ　　　.　　　. ,　　　　.　 ."
          )

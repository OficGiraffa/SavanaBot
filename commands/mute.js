...
module.exports.run = async (client, message, args) =>{
	 if(!message.member.hasPermission("MANAGE_ROLES")) return message.reply("Ops Me parece que você não tem permissão");
	    let embederro = new Discord.MessageEmbed()
            ...
            .addField(`📝 Exemplo`, `=mute <@user> 1h <motivo>`)
...
if(muterole === null) return message.reply(":x: antes de usar esse comando você precisa configurar um cargo para dar pra pessoa, **Para isso use o comando: =muterole <@cargomute>**")
let role = message.guild.roles.cache.find(role => role.id === muterole)
 
let tempo = args[1]
        if(!tempo) return message.reply("e o tempo?");
        if(!tempo.match(/^\d/)) return message.reply("o tempo que você forneceu não é um número");
 
		await(mute.roles.add(role.id));
		message.reply(`**🔇${mute.displayName}** foi mutado(a) por: ${ms(ms(tempo))},\n📝Motivo: "**${motivo}**"`);
setTimeout(function(){
            mute.roles.remove(role.id);
            message.channel.send(`**🔊${mute.displayName}** Foi desmutado(a),\n📝Motivo por ser Mutado(a): "**${motivo}**"`);
        }, ms(tempo));
}